...
(function (global) {
   ...
   * at 0.62 of a pitch it is the 0.13 em this style was drawn with, but at a
   * third of that size it is most of the line. That is what used to run a
   * fitted caption off the edge of the sheet.
   * @const {number} */
  var TRACK = 0.129;
  ...
  function toSvg(o) {
    ...
    var caption = 'SEED ' + esc(o.seed) + SEP + esc(sheet.dims) +
      SEP + esc(o.label) + SEP + esc(kind) + SEP + esc(carver) + SEP + SITE;
    var plain = 'SEED ' + o.seed + '  .  ' + sheet.dims + '  .  ' +
      o.label + '  .  ' + kind + '  .  ' + carver + '  .  ' + SITE;
    ...
    var capFont = Math.min(0.62 * span, drawnW / (plain.length * 0.78));

    var footer =
      '<text x="' + fmt((box.x0 + box.x1) / 2) + '" y="' +
      fmt(box.y1 + 2.85 * span) + '" text-anchor="middle" font-size="' +
      fmt(capFont) + '" ' + FONT + ' letter-spacing="' +
